...
public class User2ServiceController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "login":
			...
			if (u.getUid() != null) {	// uid가 존재
				...
				if (BCrypt.checkpw(pwd, u.getPwd())) {		// 비밀번호 같은지 비교(암호화해서)
					...
					request.setAttribute("url", "/jw/ch12/users2/login.jsp");
					rd = request.getRequestDispatcher("/ch12/users2/alertMsg.jsp");
					rd.forward(request, response);
				}
			} else { 			// uid가 없음
				...
				request.setAttribute("url", "/jw/ch12/users2/register.jsp");
				rd = request.getRequestDispatcher("/ch12/users2/alertMsg.jsp");
				rd.forward(request, response);
			}
			break;
		...
		case "update" :			
			...
			if (request.getMethod().equals("GET")) {
				response.sendRedirect("/jw/ch12/users2/update.jsp");
			} else {
				uid = request.getParameter("uid");
				pwd = request.getParameter("pwd");
				uname = request.getParameter("uname");
				...
				if (BCrypt.checkpw(pwd, u.getPwd())) {	// 비밀번호 일치하는지 확인
					...
					request.setAttribute("url", "/jw/ch12/users2/update");
					rd = request.getRequestDispatcher("/ch12/users2/alertMsg.jsp");
					rd.forward(request, response);
